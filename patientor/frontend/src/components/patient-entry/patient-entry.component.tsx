...
export const PatientEntry = ({entry}: PatientEntryProps)=> {
  ...
  const diagnoses = (!!entryVal.diagnosisCodes?.length && <List sx={{pl: '1em', listStyleType: 'disc', listStylePosition: 'inside',
    '& .MuiListItem-root': {display: 'list-item'}}}>
    {(entryVal.diagnosisCodes as any[]).map(d=> (<ListItem key={(d.code || d)} sx={{padding: 0}}>
      <Typography variant="body1" sx={{display: 'inline'}}>{d.code || d} {d.name}</Typography></ListItem>))}
  </List>)
    ...
    case 'OccupationalHealthcare': {
      return(<>
        ...
        {!!entry.sickLeave?.startDate && <>
          ...
          <Typography sx={{pl: '1em'}}>stard date {entry.sickLeave.startDate} - end date {entry.sickLeave.endDate} </Typography>
        </>}
        {diagnoses}
      </>)
      break
